...
const CameraFaceDetect = (props) => {
  ...
  const handleMatch = async (item) => {
    ...
    if (item.length > 0) {
      ...
      if (item[0]._label !== 'unknown') {
        if (faceName.length === 0) {
          await setFaceName(item[0]._label);

          setCountFaceMatch(1);
        } else {
          if (faceName !== item[0]._label) {
            await setFaceName(item[0]._label);
        ...
        if (countFaceMatch === 5) {
          ...
          const user = Object.values(officers)[0].find(officer => officer.OfficerName === item[0]._label);
          const getUserTimeKeeped = Log.filter(log => log._label === item[0]._label);
  ...
  const SaveImagesTimeKeeping = async (user, userCheck) => {
    ...
    try {
      ...
      userCheck[0].time = timeNow;
      userCheck[0].date = (date.getMonth() + 1) + '/' + (date.getDate()) + '/' + date.getFullYear();
      userCheck[0].avatar = IMAGES_DOMAIN + linkImages;
      userCheck[0].text = `Đã chấm công - ${shirt}`
      arr.push(userCheck[0]);
